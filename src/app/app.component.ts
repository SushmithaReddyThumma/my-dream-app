  ...
  public socialSignIn(socialPlatform : string) {
    ...
    this.socialAuthService.signIn(socialPlatformProvider).then(
      (userData) => {
        ...
        //Here we access the Mock JSON data
        this.http
        .get("/assets/mockdata.json")
        .map(data => data.json())
        .subscribe(data => {
        //this.p=data.Candidate[0].responseError[0].errorMessage;
          ...
          alert("Error Message:" + data.Candidate[0].responseError[0].errorMessage + "!    Error code is " + data.Candidate[0].responseError[0].errorCode);
